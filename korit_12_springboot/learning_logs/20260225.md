...
## 다른 요청들 보호하기
루트 패키지에 AuthenticationFilter 클래스를 생성하겠습니다. 얘는 Spring Security 상에서의 OncePerRequestFilter 인터페이스를 확장 구현하여 인증을 쓰는 doFilterInternal 메서드를 제공합니다.
```java
...
## 예외 처리하기
현재 잘못된 암호를 이용하여 로그인을 시도했을 경우를 상정하겠습니다. 현재는 403으로 나오기만 합니다.
-  Basic Auth 이용했을 때는 401이라고 안내했었는데 차이가 있습니다. 이는 적절한 로그가 안내되지 않기 때문에 협업 시에 문제가 될 수 있습니다. 그래서 예외가 발생하면 401이 뜨도록 커스텀하겠습니다.

- 루트 패키지에 AuthenticationEntryPoint를 구현 하는 AuthEntryPoint 클래스를 생성하겠습니다. implements 받고 뻘건 줄 여러분들이 없애보겠습니다.

alt text 403 -> 401로 바뀌었습니다. : response.setStatus(HttpServletResponse.SC_UNAUTHORIZED); 로 저희가 커스텀해뒀기 때문입니다. 그리고 원래 로그인 실패시에는 body 란에 아무 것도 뜨지 않았는데, Error: ?? ??? 어쩌고 떠있습니다.
- 얘는 writer.println("Error : " + authException.getMessage()); 이거 때문이겠네요. 즉, writer 객체를 생성해서 print 메서드를 호출하여 오류 로그에 대한 안내 메시지를 JSON으로 받아볼 수 있게끔 작성했습니다.

## CORS 필터추가

- CORS (Cross Origirn Resources Sharing) 클라이언트와 서버가 교차 출처 재요청을 허용하게 할 지 거부할지 결정하게하는 특정헤더도입.
backand:8080 / front:5173 으로 기본값이 잡히는데 백-프론트가 서로 통신하는 과정 상에서 요청거부가 자주 일어난다.

CORS 필터는 다른 출처에서 요청을 보내는 프론트앤드에서 필요하다.

CORS 는 요청을 가로채고, 교차 출처로 식별되면 요청에 적절한 header를 추가해준다.

Spring Security 의 CorsConfigrationSource 인터페이스이용

- 모든 출처의 HTTP 메서드와 헤더를 허용하게끔 작성할것임
...
```
특정 url 에 대해서만 요청을 허용하려면 
config.setAllowedHeaders(Arrays.asList("*"));를 수정
config.setAllowedHeaders(Arrays.asList("http://localhost:5173")); 으로


## Role -based Security

역할을 이용하여 세분화된 역할 기반 보안을 정의하는 것이 가능하며 사용자는 하나 혹은 여러개의 역할에 할당되어질수있다. 
역할은 흔히 admin, manager, user같은 계층구조를 가진다. 예로 키오스크를 기준으로 , 음료주문한다면 비회원은 역할이없거나 user에해당하며 가맹점주는 manager임, 그리고 가끔 수리하러오는사람이나 본사 사람은 admin이 된다.

이때 admin 은 manager 의 권한 대부분을 가질수 있다는점에 계층적이다.


- 역할 기반 접근 제어를 SecurityConfig에서 정의가능
특정역할이 필요한 앤드포인트를 따로 정의 

**앤드포인트에 접근하려면 admin 역할을 가지고있고 , '
**앤드포인트에 접근하려면 user역할을 필요하게끔함
    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        http
                ...
                .sessionManagement(sessionManagement
                        -> sessionManagement.sessionCreationPolicy(SessionCreationPolicy.STATELESS))
                .authorizeHttpRequests(authorizeHttpRequests
                        ->authorizeHttpRequests.requestMatchers("/admin/"),hasRole("ADMIN").requestMatchers("/users/").hasRole("USER").anyRequest().authenticated())
                .addFilterBefore(authenticationFilter,UsernamePasswordAuthenticationFilter.class)
                .exceptionHandling(exceptionHandling
                        ->exceptionHandling.authenticationEntryPoint(exceptionHandler));
